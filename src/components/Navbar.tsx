...
const Navbar: React.FC = () => {
  ...
  const mobileLinkStyle = (path: string) =>
    `nav-link px-3 py-2 rounded ${
      location.pathname === path ? "bg-light fw-bold text-primary" : "text-dark"
    }`;
  ...
  return (
    <nav className="navbar navbar-expand-lg sticky-top px-4 navbar-custom position-sticky">
        ...
        <div className="collapse navbar-collapse d-none d-lg-flex justify-content-between">
          <ul className="navbar-nav mx-auto mb-2 mb-lg-0">
            ...
            <li className="nav-item">
              <Link
                to="/about"
                className={`nav-link nav-link-custom ${isActive("/about")}`}
              >
                About
              </Link>
            </li>
            ...
            <li className="nav-item">
              <Link
                to="/blogs"
                className={`nav-link nav-link-custom ${isActive("/blogs")}`}
              >
                Blogs
              </Link>
            </li>
      ...
      <div
        ref={menuRef}
        className={`d-lg-none position-absolute end-0 me-3 mt-2 p-3 rounded-4 shadow-lg bg-white ${
      ...
      >
        <div className="d-flex flex-column gap-1">
          <Link to="/" className={mobileLinkStyle("/")} onClick={closeNav}>
            <i
              className="fa fa-home me-3 text-muted"
              style={{ width: "20px" }}
            ></i>{" "}
            Home
          </Link>
          ...
          >
            <i
              className="fa fa-info-circle me-3 text-muted"
              style={{ width: "20px" }}
            ></i>{" "}
            About
          </Link>
          <Link
            to="/categories"
            className={mobileLinkStyle("/categories")}
            onClick={closeNav}
          >
            <i
              className="fa fa-th-large me-3 text-muted"
              style={{ width: "20px" }}
            ></i>{" "}
            Categories
          </Link>
          <Link
            to="/restaurants/all"
            className={mobileLinkStyle("/restaurants/all")}
            onClick={closeNav}
          >
            <i
              className="fa fa-cutlery me-3 text-muted"
              style={{ width: "20px" }}
            ></i>{" "}
            Restaurants
          </Link>
          <Link
            to="/blogs"
            className={mobileLinkStyle("/blogs")}
            onClick={closeNav}
          >
            <i
              className="fa fa-newspaper-o me-3 text-muted"
              style={{ width: "20px" }}
            ></i>{" "}
            Blogs
          </Link>
          ...
          <div className="d-flex flex-column gap-2 mt-1">
            {user ? (
              <>
                <Link
                  to="/profile"
                  className="btn btn-outline-primary w-100 rounded-pill fw-bold btn-sm py-2"
                  onClick={closeNav}
                ...
                <button
                  className="btn btn-danger w-100 rounded-pill fw-bold btn-sm py-2"
                  onClick={closeNav}
                >
                  Logout
                </button>
              ...
              <>
                <Link
                  to="/login"
                  className="btn btn-outline-dark w-100 rounded-pill fw-bold btn-sm py-2"
                  onClick={closeNav}
                >
                  Login
                </Link>
                <Link
                  to="/register"
                  className="btn w-100 rounded-pill fw-bold text-white shadow-sm btn-sm py-2"
                  style={{ backgroundColor: "#b2744c" }}
                  onClick={closeNav}
                >
                  Register Account
                </Link>
